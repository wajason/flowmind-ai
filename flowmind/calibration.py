...
def render_report() -> str:
    ...
    L += ["", f"  資料期別：{ind.period}　單位：{ind.unit}",
          ...
          # 單位換算：total_amount 的單位是**千元**。
          # 1 億元 = 1e8 元 = 1e5 千元，因此要除以 1e5，不是 1e6。
          # 原本寫 /1e6 會把 1,810 億印成 181 億（少 10 倍），
          # 而提案書 §2.2 引用的是正確的 1,810 億 —— 兩邊對不起來。
          # 這個錯誤不會讓程式崩潰，只會讓「可重現指令」跑出來的數字
          # 與文件矛盾，正好打在本專案最重視的那一點上。
          f"（約 {ind.total_amount/1e5:,.0f} 億元／月）", ""]
