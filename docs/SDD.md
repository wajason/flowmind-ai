...
| 項目 | 結果 | 備註 |
|---|---|---|
| 決定性交叉驗證 | **五項注入缺陷全中，零漏抓**（另有 2 項非注入的額外提示，如實列出並附說明） | 負向對照組驗收 |
...
寫進 SDD 是為了讓讀者知道我們清楚邊界在哪，也讓後續開發有靶。
...
| 7 | **模型行為的版本漂移** | 模型更新後行為改變，既有 prompt 失效 | 業界靠 regression suite，但覆蓋率普遍不足 | 201 項回歸測試 + VeriFin 可隨時重跑 |
