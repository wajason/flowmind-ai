...
6 模型 × 5 面向，同一批 SROIE 資料（10 份文件 / 40 欄位）。
...
**發現 1：只看準確率會選錯模型。** `gemma4:e4b` 準確率贏 `qwen3.5:9b` 32 個
百分點，但它 40 個欄位留白 0 次、猜錯 15 次，HPES 為負。
在授信場域，一個編造的統編比一個空欄位貴太多。

**發現 2：HPES 也不能單獨看。** `qwen3.5:9b` 的 HPES 是正的（+0.150），
卻留白 25 次、只回答 37.5%。一個對每題都留白的模型 HPES 恰好是 0 ——
不難看，但毫無用處。HPES 衡量的是「回答時可不可信」，
不是「幫得上多少忙」，**必須配留白率一起讀**。
矩陣加上「留白」欄位就是為了讓這個陷阱無所遁形。

**發現 3：小模型不必然穩定。** 參數最少的 `llama3.2` 位元一致性只有 17%，
與 IBM 論文（arXiv:2511.07585）「小模型較穩定」的方向相反。
與論文一致的是「T=0 不保證可重現」，不一致的是那個方向性結論 ——
...
> ⚠️ 樣本：漂移 6 次重複、SROIE 10 份 / 40 欄位、繁中純度 4 題。
> 足以支撐「相對排序」與「硬需求是否達標」，**不足以宣稱絕對準確率**。
> 完整數據與「選非中資付出多少代價」的量化比較見
