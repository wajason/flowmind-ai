...
25 個核心模組中**只有 1 個呼叫 LLM**（`llm.py`）。這個比例本身就是架構主張。

綠色的四個層級全部零 LLM，而且各自有測試強制驗證這件事
（測試會讀模組原始碼，確認裡面沒有任何模型呼叫）——
「我們沒有用 LLM」這句話如果只是寫在文件裡，那就只是一句話。

### 4.1 供應鏈融資審查工作台（銀行／信保端）

```bash
python -m flowmind.dashboard        # → http://127.0.0.1:8000
```

![案件佇列](docs/images/dashboard-queue.png)

**入口是案件佇列，不是選單**——授信人員面對的是一批待處理案件，
依風險燈號與受理天數排序，點進去才是單一案件的細節。
這是刻意對照 Taulia／PrimeRevenue／Newgen 這類真實供應鏈金融平台的
授信工作台介面（先看待辦佇列，再挑案件處理），不是工程師測試用的
單一租戶下拉選單。

![案件細節](docs/images/dashboard-overview.png)

點進案件後五個區塊，全部取自既有模組，**呈現層不做任何運算**：

| 區塊 | 資料來源 | 重點 |
|---|---|---|
| ① 案件總覽 | `fin_alerts`（watchtower 寫入） | 紅黃綠燈；每條警示可展開**觸發它的實際資料列** |
| ② 憑證交叉驗證 | `crosscheck.run_all()` | 27 條檢查分六類，點卡片展開逐條結果 |
| ③ 現金流時間軸 | `fin_invoices` / `fin_ledger` | 見下方說明 |
| ④ 情境模擬 | `compute_cash_flow_projection()` | 重用既有算法，不新造引擎 |
| ⑤ 顧問問答與信心分數 | `evidence` 的公開權重 | **支援連續追問**；決定性與 RAG 兩種答案分開呈現 |

**每一條交叉驗證的失敗項目都可以點開看原始憑證**——`crosscheck` 的每個
`Finding` 本來就帶著 `refs`（涉及的發票／合約／流水編號，寫在程式碼裡的
原始用途就是「供人工回查」），過去這欄位算出來後直接被丟棄，畫面上只留
文字描述。現在點憑證編號會彈出視窗，直接顯示 `data/raw/{tenant}/` 底下
那筆原始記錄的完整內容——不用先自己去猜是哪個檔案、再手動搜尋比對：

![證據回查](docs/images/dashboard-evidence.png)

這不是新蓋一層資料——`GET /api/cases/{tenant}/lookup/{ref}` 純粹是拿 ref
去現有的 `receivables.json`／`contracts.json`／`bank_ledger.csv` 裡找同一個
編號，找到就照樣回傳，不做任何加工判讀。中小企業自檢頁（§4.1.1）也有
同一顆按鈕，同一支後端。
...
![新增案件](docs/images/new-case-step2.png)

**資安驗證刻意不做在這個工作台裡**——RLS 隔離、稽核雜湊鏈、Zero-Trust
輸入防護這幾項，授信人員每天不需要重新驗證一次，那是**開發期與稽核期
才需要的證據**，放進每天要用的操作介面只會混淆「這是我的工作案件」與
「這是系統健檢報告」兩種不同性質的畫面。這幾項驗證改用終端機指令
（`rag_query.py --verify-isolation` / `--verify-audit`，見 §5、§6）
與 PDF 報告／簡報呈現，仍然是**每一項都當場重新執行既有驗證函式**，
只是不常駐在日常操作畫面上。

> **為什麼現金流是上下兩張圖而不是雙 y 軸**：餘額約 1,500 萬、
> 單日入帳約 30 萬，量級差 50 倍。硬畫在同一個座標軸上就是
> 一條線貼頂、一排長條貼底，兩者都看不出變化。
> 加第二個 y 軸能解決視覺問題，但**雙軸圖可以透過調整任一軸的範圍
> 製造出任意想要的相關性** —— 在金融場合尤其不該用。
> 正確作法是兩張圖共用時間軸、各自尺度。

**「同一個問題不能有兩個答案」**：若儀表板自己算一次集中度、
終端機算另一次，兩邊有一天會不一致，而使用者無從判斷該信哪個。
測試強制驗證儀表板原始碼不得出現 `def check_` 或 `def m_`。

截圖由 `python scripts/capture_dashboard.py` 產生並進版控 ——
放示意圖任何人都做得出來，**放真實畫面才代表這套系統當下真的跑得起來**。

### 4.1.1 中小企業自檢入口（免費、獨立頁面）

```
http://127.0.0.1:8000/self-check
```

`static/self_check.html` 是**完全獨立的另一個頁面**，不是審查工作台加個開關——
使用者是不同的人（中小企業財務人員），該看到的東西也不一樣：
沒有跨案件的佇列、沒有信心分數拆解、沒有稽核雜湊鏈，只有「輸入案件編號 →
看還缺什麼 → 下載報告」。後端沿用同一支 `/api/crosscheck/{tenant}`，
下載的 PDF 也是同一支 `report.build()` 產生——**自檢報告跟送給銀行的報告
數字一定一樣**，不會有「自己修完發現銀行看到的又是另一組數字」的問題。

企業可以直接在這頁上傳／更新憑證，上傳完立即重新檢查一次，
不需要先送出去才知道少了什麼：

![送件前自檢：上傳並立即驗證](docs/images/self-check-upload.png)

### 4.2 主動監控：把被動問答變成主動秘書
...
| 項目 | 結果 |
|---|---|
| 造假偵測（22 種樣態） | Recall **99.5%**（可偵測類，含則 90.5%）· Precision 68.6% · **MCC 0.819** |
| 法規問答（105 題，2026-08-20 全量） | 整體通過 **78.1%**　·　引用率 100%　·　路由正確 100%　·　事實正確 91.4%　·　來源正確 84.6%　·　拒答正確率 **90.0%**（20 題應拒答中 18 題正確、2 題產生幻覺） |
| 引用可驗證率 CVR | **99.93%**（SROIE 361 份文件全量，2026-08-20） |
| 憑空生成率 | **0.07%**（SROIE 全量；CORD 100 份 B2B 陷阱欄位測試：600 個全數留白、0 個憑空生成） |
| 嚴格 JSON 失敗率 | **0.0%**（SROIE/FUNSD/CORD 全量） |
...
├── tests/test_core.py           201 項回歸測試（無外部依賴）
