...
**資安防護面板**（頁面下方，不屬於任何單一案件）：金管會與銀行最在意的
不是「有沒有做資安」，是「怎麼證明」——所以這個面板**每一項都是當場重新
執行一次既有的驗證函式**，不是一份靜態的宣稱清單：

![資安防護面板](docs/images/dashboard-security.png)

四張卡片依序對應：資料庫連線角色是否真的是非 superuser（RLS 才會生效）、
以另一委任案身分下無 `WHERE` 條件查詢仍看不到本案資料、647 筆稽核紀錄的
雜湊鏈逐筆重算比對、資料是否全程留在本機不經雲端 API。下方 Zero-Trust
區塊即時送出跨租戶越權、提示詞注入、規避稽核三種探測字串，當場驗證都被擋下。
...
├── tests/test_core.py           196 項回歸測試（無外部依賴）
